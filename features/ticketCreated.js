...
module.exports = (client) => {
    client.on('interactionCreate', async (interaction) => {
            ...
            if (!check) {
                const x = interaction.guild.channels.create(`${interaction.user.tag}`, {
                    ...
                    interaction.reply({
                        ...
                        ephemeral: true,
                        components: [row.addComponents(new MessageButton().setStyle('LINK').setURL(`${ticketMessage.url}`).setLabel('Go to the ticket'))]
                    })
                    })
            } else {
                interaction.reply({
        ...
        } else if (interaction.customId === 'createATicket') {
            const guild = interaction.guild;
            const user = interaction.user
            const check = guild.channels.cache.find((c) => c.topic === `${user.id}`)
            const doc = await setupSchema.findOne({guildId: interaction.guild.id})
