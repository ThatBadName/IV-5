...
module.exports = {
    ...
    callback: async ({
        ...
        try {
            ...
            const supportRoleId = doc.supportId || interaction.user.id
            ...
            if (action === 'create') {
                ...
                if (/^[0-9]{18}/g.test(userThread.topic.replace('Locked - ', ''))) {
                    ...
                    if (!logChannel) {
                        const ch = await interaction.guild.channels.create(`iv-transcripts`, {id: interaction.guild.roles.everyone, deny: ['VIEW_CHANNEL']}, {id: '980386075014991912', allow: ['SEND_MESSAGES', 'VIEW_CHANNEL', 'MANAGE_MESSAGES', 'MANAGE_CHANNELS']})
