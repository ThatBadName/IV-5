...
options: [
    ...
    {
        name: 'search-guild',
        description: 'Search the database to see if a guild is premium',
        type: 'SUB_COMMAND',
        options: [
            {
                name: 'id',
...
callback: async({interaction, client}) => {
    ...
    if (action === 'add') {
        ...
        interaction.reply({embeds: [listEmbed]})
    } else if (interaction.options.getSubcommand() === 'search-guild') {
        const result = await premuimGuildsSchema.findOne({guildId: interaction.options.getString('id')})
        if (!result) {
            const resultTimeout = await premiumTimeoutSchema.findOne({guildId: interaction.options.getString('id')})
            if (resultTimeout) return interaction.reply({embeds: [new MessageEmbed().setDescription(`The guild \`${interaction.options.getString('id')}\` has premium that expires <t:${Math.round(resultTimeout.expires.getTime() / 1000)}:R>`).setFooter({text: 'This guild is on timeout premium'})]})
            else return interaction.reply({embeds: [new MessageEmbed().setTitle('I could not find this guild in the database')]})
        }
        if (result) return interaction.reply({embeds: [new MessageEmbed().setDescription(`The guild \`${interaction.options.getString('id')}\` has premium that expires <t:${Math.round(result.expires.getTime() / 1000)}:R>`)]})
    }
}
}